...
clear all
%RR0 =
%[4.6,5,5.3,5.5,5.7,6,6.5,7,7.8,8.7,9.7,10.7,11.78,11.7,13,13.6,14.2,14.8,15.3,15.8,16.19]
%real
...
f765 = conv(R765./RR0(12),ones(p,1)/p,'same');
...
figure; 
cc= hsv(5);
plot(t-1,f1,'color',cc(1,:),'linewidth',2); hold on
plot(t-1,f2,'color',cc(2,:),'linewidth',2); hold on
plot(t-1,f3 ,'color',cc(3,:),'linewidth',2); hold on
plot(t-1,f7,'color',cc(4,:),'linewidth',2); hold on
%plot(t-1,f5,'linewidth',1.5); hold on
plot(t-1,f6,'color',cc(5,:),'linewidth',2); hold on
%plot(t-1,f7,'color',cc(6,:),'linewidth',1.5); hold on
%plot(t-1,f8,'color',cc(6,:),'linewidth',1.5); hold on
...
daspect([2.3 1 1])
    ...
    lambda_exp = [730, 740,745,750,755,760];
    %lambda_exp = [730, 740,745,750,755,760,765,775];

    mm = [min(f1), min(f2), min(f3),min(f4),min(f7),min(f6)]
...
for kk = 1: size(Te_crop,1)
    ...
    theta = 43.62; % theta = 43.62 for the minimum plots
    lambda = linspace(0.55,0.9,70)*1e-6; %chnage it to 70 for seperate plot of lambdas
    ...
    %% model on 730 740 745 750 765 nm wavelength (70 bins of lambda)
    
    figure; 
    plot(tspan(1:size(Te_avg,1))*10^12, dd(78,:),'color',cc(1,:),'linewidth',2); hold on %lambda=727
    plot(tspan(1:size(Te_avg,1))*10^12, dd(82,:),'color',cc(2,:),'linewidth',2); hold on %lambda = 737(118)
    plot(tspan(1:size(Te_avg,1))*10^12, dd(84,:),'color',cc(3,:),'linewidth',2); hold on% lambda = 742 (124)
    plot(tspan(1:size(Te_avg,1))*10^12, dd(86,:),'color',cc(4,:),'linewidth',2); hold on% lambda = 757(129)
    plot(tspan(1:size(Te_avg,1))*10^12, dd(90,:),'color',cc(5,:),'linewidth',2); hold on% lambda = 763 (131)
    ylabel('\DeltaR/R');xlabel('\Deltat(ps)'); axis tight
    set(gca,'fontsize',23);
    legend('\lambda = 730 nm','\lambda = 740 nm','\lambda = 745 nm','\lambda = 750 nm','\lambda = 760 nm')
    daspect([2.3 1 1])
...
t2 = [(-15)*dtstep (-14)*dtstep (-13)*dtstep (-12)*dtstep (-11)*dtstep (-10)*dtstep (-9)*dtstep (-8)*dtstep (-7)*dtstep (-6)*dtstep ...
    (-5)*dtstep (-4)*dtstep (-3)*dtstep (-2)*dtstep (-1)*dtstep tspan(1:size(Te_avg,1))*10^12]


    figure; 
    plot(t2-0.1, dd2(37,:),'color',cc(1,:),'linewidth',2); hold on %lambda=727
    plot(t2-0.1, dd2(39,:),'color',cc(2,:),'linewidth',2); hold on %lambda = 737(118)
    plot(t2-0.1, dd2(40,:),'color',cc(3,:),'linewidth',2); hold on% lambda = 742 (124)
    plot(t2-0.1, dd2(41,:),'color',cc(4,:),'linewidth',2); hold on% lambda = 757(129)
    plot(t2-0.1, dd2(42,:),'color',cc(5,:),'linewidth',2); hold on% lambda = 763 (131)
    ylabel('\DeltaR/R');xlabel('\Deltat(ps)'); axis tight
    set(gca,'fontsize',23);
    legend('\lambda = 730 nm','\lambda = 740 nm','\lambda = 745 nm','\lambda = 750 nm','\lambda = 760 nm')
    daspect([2.3 1 1]);ylim([-1 0]);xlim([-0.65 5])